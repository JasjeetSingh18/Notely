...
export default function EditorPage() {
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      ...
      saveTimer.current = setTimeout(() => {
        api(`/api/docs/${id}`, { method: "PUT", body: { title, contentHtml: html } }).catch(() => {});
      }, 400);
    },
  });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setMessages(chatRes.messages?.length ? chatRes.messages : [
          { role: "assistant", content: "Hi! Select text and click Ask AI, or send a chat." },
        ]);
        setLoaded(true);
      } catch {
  ...
  async function addMessages(newMessages) {
    setMessages(prev => {
      ...
      api(`/api/docs/${id}/chat`, { method: "PUT", body: { messages: updated } }).catch(console.error);
      return updated;
    });
  }
  ...
  async function askAIFromSelection(mode) {
    ...
    if (wordCount > 100) {
      setToast(`Selected text is too long (${wordCount} words). Please select a shorter section.`);
      return;
    }
    ...
    try {
      ...
      const data = res.ok ? await res.json() : { answer: `Mock ${mode} for "${selected}"` };
      ...
      await addMessages([
        { role: "assistant", content: `Mock ${mode} response for "${selected}".` },
      ]);
    } finally {
      setToast("");
    }
  ...
  function insertLatestAIResponse() {
    ...
    editor.chain()
      .focus()
      .insertContentAt(to, `<blockquote><strong>${label}:</strong> ${clean}</blockquote><p></p>`)
      .run();
...
async function enhanceSelection() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      enhancedText = data?.answer || selected;
    } else {
      enhancedText = selected; // fallback
    }
    ...
    return (
        <div className="app">
            <header className="app-header">
                ...
                <button className="btn ghost" onClick={() => askAIFromSelection("explain")}>Explain</button>
                <button className="btn ghost" onClick={() => askAIFromSelection("expand")}>Expand</button>
                <button className="btn ghost" onClick={() => askAIFromSelection("summarize")}>Summarize</button>
                <button className="btn ghost" onClick={() => askAIFromSelection("question")}>Question</button>
                <button className="btn ghost" onClick={() => askAIFromSelection("connect")}>Connect</button>
                <button className="btn primary" onClick={enhanceSelection}>Enhance</button>
                </div>
...
function formatAIAnswer(text) {
    ...
    for (const line of lines) {
        ...
        if (match) {
            if (!inList) { inList = true; result += "<ul>"; }
            result += `<li>${match[1]}</li>`;
        } else {
            if (inList) { inList = false; result += "</ul>"; }
